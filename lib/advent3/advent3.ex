defmodule Advent3 do
  ...
  def min_manhattan(wire1 \\ @wire1, wire2 \\ @wire2) do
    wire1 = String.split(wire1, ",")
    wire2 = String.split(wire2, ",")
  ...
  def min_steps(wire1 \\ @wire1, wire2 \\ @wire2) do
    wire1 = String.split(wire1, ",")
    wire2 = String.split(wire2, ",")

    wire1_path = build_path_with_steps(wire1)
    wire2_path = build_path_with_steps(wire2)
    ...
    {x, y} =
      Enum.min_by(intersection, fn {x, y} ->
        Map.get(wire1_path, {x, y}) + Map.get(wire2_path, {x, y})
      end)

    {x, y, Map.get(wire1_path, {x, y}) + Map.get(wire2_path, {x, y})}
  end

  defp build_path(wire) do
    {path, _, _} =
      Enum.reduce(
        wire,
        {MapSet.new(), 0, 0},
        fn <<dir::utf8, int::binary>>, {path, x, y} ->
          dist = String.to_integer(int)

          visited =
            case dir do
              ?R -> 1..dist |> Enum.map(fn d -> {x + d, y} end)
              ?L -> 1..dist |> Enum.map(fn d -> {x - d, y} end)
              ?U -> 1..dist |> Enum.map(fn d -> {x, y + d} end)
              ?D -> 1..dist |> Enum.map(fn d -> {x, y - d} end)
            end

          visited_mapset = MapSet.new(visited)
          {new_x, new_y} = List.last(visited)
          {MapSet.union(path, visited_mapset), new_x, new_y}
        end
      )
  ...
  defp build_path_with_steps(wire) do
    {path, _, _, _} =
      Enum.reduce(
        wire,
        {%{}, 0, 0, 0},
        fn <<dir::utf8, int::binary>>, {path, x, y, step} ->
          dist = String.to_integer(int)

          visited =
            case dir do
              ?R -> 1..dist |> Enum.map(fn d -> {x + d, y, step + d} end)
              ?L -> 1..dist |> Enum.map(fn d -> {x - d, y, step + d} end)
              ?U -> 1..dist |> Enum.map(fn d -> {x, y + d, step + d} end)
              ?D -> 1..dist |> Enum.map(fn d -> {x, y - d, step + d} end)
            end

          {new_x, new_y, last_step_count} = List.last(visited)

          path =
            Enum.reduce(visited, path, fn {x, y, step}, acc ->
              # put won't overwrite if we try to insert second time
              Map.put(acc, {x, y}, step)
            end)

          {path, new_x, new_y, last_step_count}
        end
      )
